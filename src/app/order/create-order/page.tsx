...
import { create_a_new_stripe_session } from '@/Services/common/checkout'
...
export default function Page() {
    ...
    const onSubmit: SubmitHandler<Inputs> = async data => {
        ...
        const resp =  await create_a_new_order(orderData);
        if(resp?.success){
            const finalData = {userId : user?._id,email:user?.email,orderItems: cartData?.map(item => {
                return {
                    product: item?.productID?._id,
                    qty: item?.quantity
            ...
            showToast.error({message:resp?.message,duration:5000})
            setLoader(false)
        }
